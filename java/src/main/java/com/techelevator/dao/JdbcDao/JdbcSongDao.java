...
public class JdbcSongDao implements SongDao {
	...
	public List<Song> getAllSongs() {
		...
		Song song = new Song();
		String sql = "SELECT song_id, song_name, artist, genre, user_genre, spotify_id, votes, album_art FROM song";
		try {
	...
	public Song getSongByID(int songID) {
		Song song = new Song();
		String sql = "SELECT song_id, song_name, artist, genre, user_genre, spotify_id, votes, album_art FROM song WHERE song_id = ?";
		try {
	...
	public Song getSongByName(String songName) {
		Song song = new Song();
		String sql = "SELECT song_id, song_name, artist, genre, user_genre, spotify_id, votes, album_art FROM song WHERE song_name = ?";
		try {
	...
	public Song updateSong(Song song) {
		...
		newSong.setUser_genre(song.getUser_genre());
		newSong.setSpotify_id(song.getSpotify_id());
		newSong.setVotes(song.getVotes());
		newSong.setAlbum_art(song.getAlbum_art());

		String sql = "UPDATE song SET song_name = ?, artist = ?, genre = ?, user_genre = ?, spotify_id = ?, votes = ?, album_art = ? WHERE song_id = ?";

		try {
			jdbcTemplate.update(sql, newSong.getSong_name(), newSong.getArtist(), newSong.getGenre(), newSong.getUser_genre(), newSong.getSpotify_id(), newSong.getVotes(),
					newSong.getAlbum_art(), song.getSong_id());
		} catch (CannotGetJdbcConnectionException e) {
			throw new DaoException("Unable to connect to server or database", e);
		}
		return getSongByName(newSong.getSong_name());
